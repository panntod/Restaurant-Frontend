    ...
    return (
        <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <figure className='w-96 h-60 rounded-t-lg overflow-hidden'>
                <img className="w-full" src={`${api_url}/images/${data.image}`} alt={data.image} />
            </figure>
            <div className="p-5">
                <H4>{data.name}</H4>
                ...
                <div className="flex justify-end items-center gap-4">
                    <button onClick={() => updateCart(data.id, "decrease")} className="w-8 h-8 font-bold rounded-full bg-red-600 hover:bg-red-500 text-white">-</button>
                    {count}
                    <button onClick={() => updateCart(data.id, "increase")} className="w-8 h-8 font-bold rounded-full bg-green-600 hover:bg-green-500 text-white">+</button>
                </div>
            </div>
        </div>
